...
describe('DeleteReservationUseCase', () => {
  ...
  it('should delete old rows in both sheets when cutoff date exists', async () => {
    ...
    googleSheetsServiceMock.deleteClosedSlotsBefore.mockResolvedValue(3);
    ...
    expect(googleSheetsServiceMock.deleteClosedSlotsBefore).toHaveBeenCalledWith(
      'lunes 02 de marzo 2030 02/03/2030',
    );
  ...
  it('should skip deletion when there are no rows in the availability sheet', async () => {
    ...
    expect(googleSheetsServiceMock.deleteClosedSlotsBefore).not.toHaveBeenCalled();
  });
